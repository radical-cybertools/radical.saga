...
_SCRIPTS = {
    'ssh' : { 
        ...
        # scp
        'copy_to'       : '%(scp_env)s "%(scp_exe)s"   %(scp_args)s  %(cp_flags)s %(s_flags)s "%(src)s" "%(scp_root)s%(tgt)s"',
        'copy_from'     : '%(scp_env)s "%(scp_exe)s"   %(scp_args)s  %(cp_flags)s %(s_flags)s "%(scp_root)s%(src)s" "%(tgt)s"',
        'copy_to_in'    : '',
        'copy_from_in'  : '',
      # # sftp
