...
def main():
    ...
    ec2keypair_ctx.user_key = '/Users/oweidner/.ssh/rsa_ec2.pub'
    ec2keypair_ctx.user_id  = 'root'  # the user id on the target VM

    # TODO
    ssh_ctx = saga.Context ('ssh')
    ssh_ctx.user_id   = 'root'
    ssh_ctx.user_cert = '/Users/oweidner/.ssh/rsa_ec2.pub'

    session = saga.Session(False)  # FALSE: don't use any other (default) contexts
